...
fn parses_user_type_with_type_arguments() {
    ...
    assert_eq!(segments[0].first_token().unwrap().kind(), IDENTIFIER);
    assert_eq!(segments[1].first_token().unwrap().kind(), IDENTIFIER);
...
fn parses_function_type_without_receiver() {
    ...
    assert_eq!(ret.first_token().unwrap().kind(), IDENTIFIER);
}
...
fn parses_function_type_with_receiver() {
    ...
    assert_eq!(receiver.first_token().unwrap().kind(), IDENTIFIER);
}
